...
export class WithdrawIntentService {
  constructor(
    @InjectModel('withdrawalIntent') private withdrawlIntent: Model<any>,
    @InjectModel('CharityAppUsers') private user: Model<any>,
    private readonly paymentservice: PaymentService,
  ) {}

  async createWithdrawlIntent({
    userId,
    amount,
    currency,
    res,
    ...
    try {
      ...
      const WI = await this.withdrawlIntent.create({
        userId,
        userName,
        amount,
        currency,
  ...
  async cancelWithdrawlIntent({
    ...
    try {
      ...
      const withdrawalIntent = await this.withdrawlIntent.findOneAndUpdate({
      ...
      const WI = await this.withdrawlIntent.findOneAndUpdate(
        {
          _id: withdrawalIntentId,
        },
        {
